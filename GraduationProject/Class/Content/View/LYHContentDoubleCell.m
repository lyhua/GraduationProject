...
@implementation LYHContentDoubleCell
...
-(void)setupPictures
{
    _doubleView = [[LYHDoublePicturesView alloc] init];
    [self.contentView addSubview:_doubleView];
    //进行图片视图布局
    [_doubleView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.contentView.mas_left);
        make.right.equalTo(self.contentView.mas_right);
...
#pragma mark 懒加载图片视图
-(LYHDoublePicturesView *)doubleView
{
    if(!_doubleView)
    {
        LYHDoublePicturesView *doubleView = [[LYHDoublePicturesView alloc] initWithpicturesModel:self.pictures];
        [self.contentView addSubview:doubleView];
        _doubleView = doubleView;
    }
    return _doubleView;
}
...
- (void)layoutSubviews
{
    ...
    //有图片模型才进行布局
    if (_pictures) {
        self.doubleView.frame = self.content.middleFrame;
    }
}
