...
@implementation LYHContentSingleCell
...
-(void)setupPictures
{
    _singleView = [[LYHSinglePicturesView alloc] init];
...
-(void)commentBtnClick
{
    ...
    [ImageSize changeWithImageSize:self.imageSize];
    //发送通知
    [[NSNotificationCenter defaultCenter] postNotificationName:LYHCellCommentBtnClickNotification object:nil];
}

#pragma mark 懒加载图片视图
-(LYHSinglePicturesView *)singleView
{
    if(!_singleView)
    {
//        LYHSinglePicturesView *singleView = [[LYHSinglePicturesView alloc] initWithpicturesModel:self.pictures];
        LYHSinglePicturesView *singleView = [[LYHSinglePicturesView alloc] initWithImageSize:self.imageSize pictures:self.pictures];
...
- (void)layoutSubviews
{
    ...
    //有图片模型才进行布局
    if (_pictures) {
        self.singleView.frame = self.content.middleFrame;
    }
}
